...
and seekReadCustomAttrUncached ctxtH (mdReader: System.Reflection.Metadata.MetadataReader) (CustomAttrIdx (cat, idx, valIdx)) = 
    ...
    try
        let tableIndex =
            match cat with
            | x when x = cat_MethodDef -> TableIndex.MethodDef
            | x when x = cat_MemberRef -> TableIndex.MemberRef
            | _ -> failwith "Invalid Cat"
        let handle = MetadataTokens.EntityHandle(tableIndex, idx)
        let ilMethodDef = ILReader.convertEntityHandleToILMethodDef mdReader handle
        if method.DeclaringType.TypeRef.FullName.Contains("Beef") || method.DeclaringType.TypeRef.FullName.Contains("Class1") then
            printfn "%A" method.Name
            printfn "%A"ilMethodDef
    with
    | ex -> 
        printfn "%A" ex
