module internal FSharp.Compiler.CodeAnalysis.AstVisitor
    ...
    member this.AdjustedRange =
        ...
        | ParsedInput.ImplFile (ParsedImplFileInput (modules=modules)) ->
            ...
            | _ ->
                ...
                |> List.reduce (unionRanges)
        | ParsedInput.SigFile (ParsedSigFileInput (modules=_modules)) ->
            range0 // TODO:
...
type AstVisitor<'T> () as this =
    ...
    default this.VisitParsedInput parsedInput =
        ...
        | ParsedInput.ImplFile (ParsedImplFileInput (_, _, _, _, _, modules, _)) -> 
            ...
            |> tryVisitList
        | ParsedInput.SigFile (ParsedSigFileInput (_, _, _, _, _modules)) -> 
            None
    ...
    default this.VisitComponentInfo info =
        match info with
        | ComponentInfo(attribs, typarDecls, constraints, longId, _, _, _, _m) ->
            let attribs =
                attribs
                |> mapVisitList (fun x -> x.Range) this.VisitAttributeList
    ...
    default this.VisitTypeDefnSimpleRepr simpleRepr =
        ...
        | SynTypeDefnSimpleRepr.General (typeDefnKind, tys, valSigs, fields, _, _, patsOpt, _m) ->
            let typeDefnKind =
                [typeDefnKind]
                |> mapVisitList (fun x -> x.PossibleRange) this.VisitTypeDefnKind
    ...
    default this.VisitSimplePat simplePat =
        ...
        | SynSimplePat.Id (id, simplePatInfoOpt, _, _, _, _m) ->
            let id =
                [id]
                |> mapiVisitList (fun x -> x.idRange) this.VisitIdent
    ...
    default this.VisitEnumCase enumCase =
        match enumCase with
        | EnumCase (attribs, id, sconst, _, _m) ->
            let attribs =
                attribs
                |> mapVisitList (fun x -> x.Range) this.VisitAttributeList
    ...
    default this.VisitValSig valSig =
        match valSig with
        | ValSpfn (attribs, id, valTyparDecls, ty, valInfo, _, _, _, _, exprOpt, _m) ->
            let attribs =
                attribs
                |> mapVisitList (fun x -> x.Range) this.VisitAttributeList
