...
type FSharpCheckFileResults(filename: string, errors: FSharpErrorInfo[], scopeOptX: TypeCheckInfo option, dependencyFiles: string[], reactorOpsX:IReactorOperations, keepAssemblyContents: bool) =

    // This may be None initially, or may be set to None when the object is disposed or finalized
    let mutable details = match scopeOptX with None -> None | Some scopeX -> Some (scopeX, reactorOpsX)
...
type BackgroundCompiler(legacyReferenceResolver, projectCacheSize, keepAssemblyContents, keepAllBackgroundResolutions, tryGetMetadataSnapshot, suggestNamesForErrors) as self =
    ...
    let MakeCheckFileResultsEmpty(filename, creationErrors) = 
        FSharpCheckFileResults (filename, creationErrors, None, [| |], reactorOps, keepAssemblyContents)

    let MakeCheckFileResults(filename, options:FSharpProjectOptions, scope, dependencyFiles, creationErrors, parseErrors, tcErrors) = 
        let errors = 
        ...
        FSharpCheckFileResults (filename, errors, Some scope, dependencyFiles, reactorOps, keepAssemblyContents)

    let MakeCheckFileAnswer(filename, tcFileResult, options:FSharpProjectOptions, dependencyFiles, creationErrors, parseErrors, tcErrors) = 
        match tcFileResult with 
        | Parser.TypeCheckAborted.Yes  ->  FSharpCheckFileAnswer.Aborted                
        | Parser.TypeCheckAborted.No scope -> FSharpCheckFileAnswer.Succeeded(MakeCheckFileResults(filename, options, scope, dependencyFiles, creationErrors, parseErrors, tcErrors))
    ...
    member bc.GetBackgroundParseResultsForFileInProject(filename, options, userOpName) =
        reactor.EnqueueAndAwaitOpAsync(userOpName, "GetBackgroundParseResultsForFileInProject ", filename, fun ctok -> 
            cancellable {
                let! builderOpt, creationErrors = getOrCreateBuilderAndKeepAlive (ctok, options, userOpName)
                match builderOpt with
...
//            | Some (parseResults, checkResults, _, _) when builder.AreCheckResultsBeforeFileInProjectReady(filename) -> 
                            ...
                            try
                                ...
                                let! tcErrors, tcFileResult = 
                                    Parser.CheckOneFile(parseResults, sourceText, fileName, options.ProjectFileName, tcPrior.TcConfig, tcPrior.TcGlobals, tcPrior.TcImports, 
                                                        tcPrior.TcState, tcPrior.ModuleNamesDict, loadClosure, tcPrior.TcErrors, reactorOps, (fun () -> true), textSnapshotInfo, userOpName, suggestNamesForErrors)
                                ...
                                let checkAnswer = MakeCheckFileAnswer(fileName, tcFileResult, options, Array.ofList tcPrior.TcDependencyFiles, creationErrors, parseResults.Errors, tcErrors)
    ...
    member bc.CheckFileInProject(parseResults: FSharpParseFileResults, filename, fileVersion, sourceText: ISourceText, options, textSnapshotInfo, userOpName) =
        ...
        async {
            try 
                if implicitlyStartBackgroundWork then 
                    reactor.CancelBackgroundOp() // cancel the background work, since we will start new work after we're done
                let! builderOpt,creationErrors = execWithReactorAsync (fun ctok -> getOrCreateBuilderAndKeepAlive (ctok, options, userOpName))
                match builderOpt with
            ...
            | Some builder -> 
                ...
                let scope = 
                    TypeCheckInfo(tcProj.TcConfig, tcProj.TcGlobals, 
                                  ...
                                  loadClosure, reactorOps, (fun () -> true), None, 
                                  tcProj.LatestImplementationFile,
                                  List.head tcProj.TcOpenDeclarationsRev)     
                let typedResults = MakeCheckFileResults(filename, options, scope, Array.ofList tcProj.TcDependencyFiles, creationErrors, parseResults.Errors, tcErrors)
                return (parseResults, typedResults)
           })
    ...
    member private bc.TryGetLogicalTimeStampForProject(cache, ctok, options, userOpName: string) =
        ...
        let builderOpt,_creationErrors = getOrCreateBuilderAndKeepAlive (ctok, options, userOpName + ".TryGetLogicalTimeStampForProject") |> Cancellable.runWithoutCancellation
        match builderOpt with 
        | None -> None
        ...
        async {
            ...
            return
                ...
                | Parser.TypeCheckAborted.No tcFileInfo ->
                    ...
                    let typeCheckResults = FSharpCheckFileResults (filename, errors, Some tcFileInfo, dependencyFiles, reactorOps, false)   
                    let projectResults = 
