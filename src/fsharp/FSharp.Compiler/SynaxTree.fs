...
let private failSlot () = failwith "invalid slot"

#nowarn "57"

type FSharpSyntaxTokenKind = FSharp.Compiler.SourceCodeServices.Lexer.FSharpSyntaxTokenKind
type FSharpSyntaxToken = FSharp.Compiler.SourceCodeServices.Lexer.FSharpSyntaxToken

[<Struct>]
type FSharpRange internal (range: range) =
    ...
    static member Combine (range1: FSharpRange, range2: FSharpRange) =
        FSharpRange(FSharp.Compiler.Range.unionRanges range1.InternalRange range2.InternalRange)

[<Struct>]
type FSharpSyntaxNodeOrToken private (parent: FSharpSyntaxNode, nodeOrToken: obj, tokenRange: FSharpRange) =
    
    member _.IsNode =
        match nodeOrToken with
        | :? FSharpSyntaxNode -> true
        | :? token -> false
        | _ -> failwith "invalid node or token"

    member this.IsToken = not this.IsNode

    member _.Parent = parent

    member _.Range = 
        match nodeOrToken with
        | :? FSharpSyntaxNode as node -> node.Range
        | :? token -> tokenRange
        | _ -> failwith "invalid node or token"
    ...
    member this.Token =
        if this.IsNode then
            ...
            FSharpSyntaxToken(nodeOrToken :?> token, tokenRange.InternalRange)

    member this.Node =
        if this.IsNode then
            nodeOrToken :?> FSharpSyntaxNode
        else
            Unchecked.defaultof<FSharpSyntaxNode>

    static member internal CreateNode(node: FSharpSyntaxNode) =
        FSharpSyntaxNodeOrToken(Unchecked.defaultof<_>, node, FSharpRange())

    static member internal CreateToken(parent: FSharpSyntaxNode, token: FSharpSyntaxToken) =
        FSharpSyntaxNodeOrToken(parent, token.tok, FSharpRange token.tokRange)

let private mkNode node = FSharpSyntaxNodeOrToken.CreateNode node
let private mkToken parent token = FSharpSyntaxNodeOrToken.CreateToken(parent, token)
...
type FSharpSyntaxNode internal (parent: FSharpSyntaxNode) =
    ...
    member this.GetChildCount() =
        let mutable count = 0
        for i = 0 to this.SlotCount - 1 do
            count <- count + this.GetSlotLength i
        count

    member this.GetSlot(index: int) =
        let mutable slot = -1
        let mutable offset = 0
        for i = 0 to this.SlotCount - 1 do
            if slot = -1 then
                let nextAcc = offset + this.GetSlotLength i
                if index < nextAcc && slot = -1 then
                    slot <- i
                else
                    offset <- offset + this.GetSlotLength i
        struct(slot, offset)
    
    abstract Range : FSharpRange

    abstract SlotCount : int

    abstract GetSlotLength : slot: int -> int

    abstract GetChildNodeOrToken : index: int -> FSharpSyntaxNodeOrToken

type FSharpSyntaxNode with

    member this.GetAllTokens() =
        seq {
            for i = 0 to this.GetChildCount() - 1 do
                let nodeOrToken = this.GetChildNodeOrToken i
                if nodeOrToken.IsToken then
                    yield nodeOrToken.Token
                else
                    yield! nodeOrToken.Node.GetAllTokens() }
        ...
        seq {
            for i = 0 to this.GetChildCount() - 1 do
                ...
                if nodeOrToken.IsNode then
                    yield nodeOrToken.Node
                    yield! nodeOrToken.Node.GetAllNodes() }

[<Struct>]
type FSharpSyntaxList<'T when 'T :> FSharpSyntaxNode> internal (nodes: 'T []) =
    ...
    member _.Range =
        ...
        | [||] -> FSharpRange range0
        | [|x|] -> x.Range
        | _ -> FSharpRange.Combine(nodes.[0].Range, nodes.[nodes.Length - 1].Range)

    member _.Length = nodes.Length

    member _.Item with get index = nodes.[index]

[<AbstractClass>]
type FSharpSyntaxTree internal (internalNode: ParsedInput) =
    inherit FSharpSyntaxNode(Unchecked.defaultof<_>)

    member internal _.InternalNode = internalNode

    override this.Range = FSharpRange this.InternalNode.Range

    static member Create(internalNode: ParsedInput) =
        match internalNode with
        | ParsedInput.ImplFile _ -> FSharpSyntaxTreeImplementation(internalNode) :> FSharpSyntaxTree
        | ParsedInput.SigFile _ -> FSharpSyntaxTreeSignature(internalNode) :> FSharpSyntaxTree

let inline private lazyNode<'T when 'T :> FSharpSyntaxNode> (value: byref<'T>) f =
    match box value with
    | null -> value <- f ()
    | _ -> ()
    value 

let inline private lazyNodeList<'T when 'T :> FSharpSyntaxNode> (value: byref<FSharpSyntaxList<'T>>) f =
    match box value.Nodes with
    | null -> value <- f ()
    | _ -> ()
    value 

[<Sealed>]
type FSharpSyntaxTreeImplementation internal (internalNode: ParsedInput) =
    inherit FSharpSyntaxTree(internalNode)

    let mutable implementation : FSharpSyntaxImplementation = Unchecked.defaultof<_>

    member this.Implementation =
        lazyNode &implementation (
            fun () ->
    ...
    override _.SlotCount = 1

    override _.GetSlotLength slot =
        match slot with
        | 0 -> 1
        | _ -> failSlot ()

    override this.GetChildNodeOrToken index =
        match this.GetSlot index with
        | 0, _ -> FSharpSyntaxNodeOrToken.CreateNode(this.Implementation)
        | _ -> failIndex ()
            
[<Sealed>]
type FSharpSyntaxTreeSignature internal (internalNode: ParsedInput) =
    inherit FSharpSyntaxTree(internalNode)

  //  let mutable signature : FSharpSyntaxSignature = Unchecked.defaultof<_>

    //member this.Signature =
    //    lazyNode &signature (fun () ->
    //        match this.InternalNode with
    ...
    override _.GetChildNodeOrToken index = failIndex ()
        //match index with
...
type FSharpSyntaxImplementation internal (parent: FSharpSyntaxNode, internalNode: ParsedImplFileInput) =
    ...
    let mutable pragmas : FSharpSyntaxList<FSharpSyntaxPragma> = Unchecked.defaultof<_>
    let mutable hashDirectives : FSharpSyntaxList<FSharpSyntaxHashDirective> = Unchecked.defaultof<_>
    let mutable moduleOrNamespaces : FSharpSyntaxList<FSharpSyntaxModuleOrNamespace> = Unchecked.defaultof<_>

    member internal _.InternalNode = internalNode

    member this.Pragmas =
        lazyNodeList &pragmas (fun () ->
            match this.InternalNode with
            | ParsedImplFileInput.ParsedImplFileInput (_, _, _, scopedPragmas, _, _, _) ->
                scopedPragmas
                |> List.map (fun x -> FSharpSyntaxPragma.InternalCreate(this, x))
                |> Array.ofList
                |> FSharpSyntaxList
        )

    member this.HashDirectives =
        lazyNodeList &hashDirectives (fun () ->
            match this.InternalNode with
            | ParsedImplFileInput.ParsedImplFileInput (_, _, _, _, hashDirectives, _, _) ->
                hashDirectives 
                |> List.map (fun x -> FSharpSyntaxHashDirective(this, x))
                |> Array.ofList
                |> FSharpSyntaxList
        )

    member this.ModuleOrNamespaces =
        lazyNodeList &moduleOrNamespaces (fun () ->
            match this.InternalNode with
            | ParsedImplFileInput.ParsedImplFileInput (_, _, _, _, _, modules, _) ->                   
                modules 
                |> List.map (fun x -> FSharpSyntaxModuleOrNamespace(this, x))
                |> Array.ofList
                |> FSharpSyntaxList
        )
    ...
    override _.SlotCount = 3

    override this.GetSlotLength slot =
        match slot with
        | 0 -> this.Pragmas.Length
        | 1 -> this.HashDirectives.Length
        | 2 -> this.ModuleOrNamespaces.Length
        | _ -> failSlot ()

    override this.GetChildNodeOrToken index =
        match this.GetSlot index with
        | 0, _ -> mkNode this.Pragmas.[index]
        | 1, offset -> mkNode this.HashDirectives.[index - offset]
...
//    let mutable pragmas : FSharpSyntaxNodeList<FSharpSyntaxPragma> = Unchecked.defaultof<_>
//    let mutable hashDirectives : FSharpSyntaxNodeList<FSharpSyntaxHashDirective> = Unchecked.defaultof<_>
//    let mutable moduleOrNamespaceSignatures : FSharpSyntaxNodeList<FSharpSyntaxModuleOrNamespaceSignature> = Unchecked.defaultof<_>

//    member internal _.InternalNode = internalNode

//    member this.Pragmas =
//        lazyNode &pragmas (fun () ->
//            match this.InternalNode with
//            | ParsedSigFileInput.ParsedSigFileInput (_, _, scopedPragmas, _, _) ->
//                let nodes = FSharpSyntaxNodeList(this)
//                nodes.Nodes <-
//                    scopedPragmas
//                    |> List.map (fun x -> FSharpSyntaxPragma.InternalCreate(nodes, x))
//                    |> Array.ofList
//                nodes
//        )

//    member this.HashDirectives =
//        lazyNode &hashDirectives (fun () ->
//            match this.InternalNode with
//            | ParsedSigFileInput.ParsedSigFileInput (_, _, _, hashDirectives, _) ->
//                let nodes = FSharpSyntaxNodeList(this)
//                nodes.Nodes <-
//                    hashDirectives 
//                    |> List.map (fun x -> FSharpSyntaxHashDirective(nodes, x))
//                    |> Array.ofList
//                nodes
//        )

//    member this.ModuleOrNamespaces =
//        lazyNode &moduleOrNamespaceSignatures (fun () ->
//            match this.InternalNode with
//            | ParsedSigFileInput.ParsedSigFileInput (_, _, _, _, modules) ->
//                let nodes = FSharpSyntaxNodeList(this)
//                nodes.Nodes <-                    
//                    modules 
//                    |> List.map (fun x -> FSharpSyntaxModuleOrNamespaceSignature(nodes, x))
//                    |> Array.ofList
//                nodes
//        )
...
//                FSharpSyntaxNodeList<FSharpSyntaxHashDirective>(this, item3)
//            | _ -> failwith "invalid syntax" 

//    member this.item4 =
//            match this.InternalNode with
//            | ParsedSigFileInput.ParsedSigFileInput (_, _, _, _, item4) ->
//                FSharpSyntaxNodeList<FSharpSyntaxModuleOrNamespaceSignature>(this, item4)
//            | _ -> failwith "invalid syntax" 

//    override _.Range = FSharpRange internalNode.Range
...
[<AbstractClass>]
type FSharpSyntaxPragma internal (parent: FSharpSyntaxNode, internalNode: ScopedPragma) =
    inherit FSharpSyntaxNode(parent)
    ...
    static member internal InternalCreate(parent, internalNode) =
        match internalNode with
        | ScopedPragma.WarningOff _ -> FSharpSyntaxPragmaNoWarn(parent, internalNode) :> FSharpSyntaxPragma

[<Sealed>]
type FSharpSyntaxPragmaNoWarn internal (parent: FSharpSyntaxNode, internalNode: ScopedPragma) =
    inherit FSharpSyntaxPragma(parent, internalNode)

    member this.WarningNumber =
        match this.InternalNode with
        | ScopedPragma.WarningOff (_, warningNumber) -> warningNumber

    override _.SlotCount = 0

    override _.GetSlotLength _ = failSlot ()

    override _.GetChildNodeOrToken _ = failIndex ()
...
type FSharpSyntaxHashDirective internal (parent: FSharpSyntaxNode, internalNode: ParsedHashDirective) =
    ...
    member this.Name =
        match this.InternalNode with
        | ParsedHashDirective (name, _, _) -> name

    member this.Arguments =
        match this.InternalNode with
        | ParsedHashDirective (_, args, _) -> args
...
let private longIdentToLongIdentWithDots (lid: LongIdent) =
    let estimatedDotRanges =
        lid
        |> List.map (fun x -> mkFileIndexRange x.idRange.FileIndex x.idRange.End (mkPos x.idRange.EndLine (x.idRange.EndColumn + 1)))
        |> List.take (lid.Length - 1)
    LongIdentWithDots(lid, estimatedDotRanges)
            
//[<AbstractClass>]
type FSharpSyntaxModuleOrNamespace internal (parent: FSharpSyntaxNode, internalNode: SynModuleOrNamespace) =
    inherit FSharpSyntaxNode(parent)

    let mutable name : FSharpSyntaxName = Unchecked.defaultof<_>
    ...
    member this.ModuleToken = 
        let r = this.InternalNode.Range
        FSharpSyntaxToken(MODULE, mkFileIndexRange r.FileIndex r.Start (mkPos r.EndLine (r.EndColumn + 6)))

    member this.Name =
        lazyNode &name (fun () ->
            match this.InternalNode with
            | SynModuleOrNamespace.SynModuleOrNamespace (lid, _, _, _, _, _, _, _) ->
                FSharpSyntaxName.Create(this, longIdentToLongIdentWithDots lid)
        )
    ...
    override this.GetChildNodeOrToken index =
        match this.GetSlot index with
        | 0, _ -> mkToken this this.ModuleToken
        | 1, _ -> mkNode this.Name
        | _ -> failIndex ()
            
//[<AbstractClass>]
//type FSharpSyntaxModuleOrNamespaceSignature internal (parent: FSharpSyntaxNode, internalNode: SynModuleOrNamespaceSig) =
//    inherit FSharpSyntaxNode(parent)

//    let mutable __item0 : FSharpSyntaxNodeList<FSharpSyntaxName> = Unchecked.defaultof<_>
//    let mutable __item2 : FSharpSyntaxModuleOrNamespaceKind = Unchecked.defaultof<_>
...
[<AbstractClass>]
type FSharpSyntaxName internal (parent: FSharpSyntaxNode) =
    inherit FSharpSyntaxNode(parent)

    static member internal Create(parent: FSharpSyntaxNode, internalNode: LongIdentWithDots) =
        match internalNode with
        | LongIdentWithDots (lid, dotms) ->
            match List.rev lid, List.rev dotms with
            | [], _ -> failwith "invalid name"
            | [id], [] -> FSharpSyntaxIdentifier(parent, id) :> FSharpSyntaxName
            | id :: lidTail, dotm :: dotmTail ->
                let left = fun parent -> FSharpSyntaxName.Create(parent, LongIdentWithDots(List.rev lidTail, List.rev dotmTail))
                let right = fun parent -> FSharpSyntaxIdentifier(parent, id) :> FSharpSyntaxSimpleName
                FSharpSyntaxQualifiedName(parent, left, dotm, right) :> FSharpSyntaxName
            | _ ->
                failwith "invalid name"

[<AbstractClass>]
type FSharpSyntaxSimpleName internal (parent: FSharpSyntaxNode) =
    inherit FSharpSyntaxName(parent)

    abstract IdentifierToken : FSharpSyntaxToken

    static member internal Create(parent: FSharpSyntaxNode, internalNode: Ident) =
        FSharpSyntaxIdentifier(parent, internalNode) :> FSharpSyntaxSimpleName

[<Sealed>]
type FSharpSyntaxIdentifier internal (parent: FSharpSyntaxNode, internalNode: Ident) =
    inherit FSharpSyntaxSimpleName(parent)

    member internal _.InternalNode = internalNode

    override this.IdentifierToken = FSharpSyntaxToken(IDENT this.InternalNode.idText, this.InternalNode.idRange)

    override this.Range = FSharpRange this.InternalNode.idRange
    ...
    override this.GetChildNodeOrToken index =
        match this.GetSlot index with
        | 0, _ -> mkToken this this.IdentifierToken
        | _ -> failIndex ()

[<Sealed>]
type FSharpSyntaxQualifiedName internal (parent: FSharpSyntaxNode, left: FSharpSyntaxNode -> FSharpSyntaxName, dotToken: range, right: FSharpSyntaxNode -> FSharpSyntaxSimpleName) =
    inherit FSharpSyntaxName(parent)

    member this.Left = left this

    member _.DotToken = FSharpSyntaxToken(token.DOT, dotToken)

    member this.Right = right this

    override this.Range = FSharpRange.Combine(this.Left.Range, this.Right.Range)
    ...
    override this.GetChildNodeOrToken index =
        match this.GetSlot index with
        | 0, _ -> mkNode this.Left
        | 1, _ -> mkToken this this.DotToken
        | 2, _ -> mkNode this.Right
        | _ -> failIndex ()
