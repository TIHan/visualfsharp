...
type internal FsiDynamicCompiler
                        ...
                        if v.IsModuleValueOrMember && not v.IsMember then
                            let fsiValueOpt =
                                ...
                                | Item.Value vref ->
                                    ...
                                    match optValue with
                                    | Some (res, ty) -> 
                                        boundValues <- boundValues |> NameMap.add v.LogicalName vref.Deref
                                        Some(FsiValue(res, ty, FSharpType(cenv, vref.Type)))
                                    | None -> 
                                        None 
                                | _ -> None
    ...
    member __.GetBoundValues (istate) =
        ...
        [ for pair in istate.boundValues do
            ...
            | Some (res, ty) ->
                yield (nm, FsiValue(res, ty, FSharpType(cenv, v.Type)))
            | _ ->
                () ]
    ...
    member __.AddBoundValue (ctok, errorLogger: ErrorLogger, istate, name: string, value: obj, typeAnnotationOpt: string option) =
        ...
        | _ ->
            ...
            let tcEnvAtEndOfLastInput = 
                TypeChecker.AddLocalSubModule istate.tcGlobals amap range0 istate.tcState.TcEnvFromImpls moduleOrNamespace
                |> TypeChecker.AddLocalVal TcResultsSink.NoSink range0 v
            let tcState = istate.tcState.NextStateAfterIncrementalFragment tcEnvAtEndOfLastInput
            let tcConfig = TcConfig.Create(tcConfigB,validate=false)

            let impl =
                let bindExpr = TypedTreeOps.mkUncheckedDefaultOfLibCall istate.tcGlobals ty range0
                let binding = Binding.TBind(v, bindExpr, NoDebugPointAtLetBinding)
                let mbinding = ModuleOrNamespaceBinding.Module(moduleOrNamespace, TMDefs([TMDefLet(binding, range0)]))
                let expr = ModuleOrNamespaceExprWithSig(mty, TMDefs([TMDefs[TMDefRec(false, [], [mbinding], range0)]]), range0)
                TypedImplFile.TImplFile(QualifiedNameOfFile.QualifiedNameOfFile(Ident(fragName, range0)), [], expr, false, false, StampMap.Empty)
