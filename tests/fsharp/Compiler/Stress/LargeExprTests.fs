...
[<EntryPoint>]
let main _ =
    System.Console.WriteLine(System.Linq.Enumerable.Sum(s(), fun c -> int c)) |> ignore
    0
"""
        let source = new StringBuilder();
        source.Append(source0) |> ignore
        ...
        source.Append(source1) |> ignore

        let fsCmpl =
            Compilation.Create(source.ToString(), Fs, Exe, options = [|"--optimize-"|])

        CompilerAssert.ExecutionHasOutput(fsCmpl, "58430604")